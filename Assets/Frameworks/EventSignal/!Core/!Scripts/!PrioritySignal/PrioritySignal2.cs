namespace HandyPackage
{
    ...
    public class PrioritySignal<T1, T2>
    {
        private SortedDictionary<int, List<Func<T1, T2, bool>>> actionQueues = new SortedDictionary<int, List<Func<T1, T2, bool>>>();

        public IDisposable Listen(Func<T1, T2, bool> action, int priority)
        {
            if (action == null)
            {
            ...
            {
                actionQueues.Add(priority, new List<Func<T1, T2, bool>>());
            }
        ...
        public bool Fire(T1 param1, T2 param2)
        {
            ...
            foreach (var item in actionQueues)
            {
                ...
                {
                    if (!item.Value[i].Invoke(param1, param2)) return false;
                }
            }
            return true;
        }
